...
timer_init (void)
{
  list_init(&sleeping); //initialize the sleeping list
...
timer_sleep (int64_t ticks)
{
  ...
  struct thread *current = thread_current(); //the current thread to be queued during sleep time
  enum intr_level old = intr_disable(); //disable interrupts for critical section
  current -> wakeup = start + ticks; //save the time for the thread to wake-up at
  list_insert_ordered(&sleeping, &current -> elem, list_less, NULL); //insert the threads into the list by order in respect to which has less wake-up time
